...
import { X, Sparkles, MapPin, Building, Calendar, Info, Heart, Compass, Star, Check } from "lucide-react";
...
export default function SearchPage() {
  ...
  // TripAdvisor-style interactive details modal state
  ...
  const handleAiRecommend = () => {
    if (!query) {
      alert("Please enter what kind of trip you want in the search box first! (e.g. 'peaceful beach')");
      return;
    }
    setAiLoading(true);
    setAiResult(null);
    setTimeout(() => {
      const match = CITIES.find(c => c.tags.some(t => query.toLowerCase().includes(t)) || c.description.toLowerCase().includes(query.toLowerCase())) || CITIES[3];
      setAiResult(`✨ AI Recommendation: Based on your search for "${query}", you should definitely visit **${match.name}, ${match.country}**! ${match.description}`);
      setAiLoading(false);
    }, 1500);
  };

  // Instant One-Click AI Trip Planner Action
  const generateCityTrip = async (city) => {
    ...
    try {
      ...
      console.error("Error generating trip from Explore:", err);
  ...
  const filtered = CITIES.filter(c => {
    ...
    const matchF = filter === "all" || c.cost_index.toLowerCase().replace("-range", "") === filter;
    ...
    <div className="page-container">
      ...
      <div style={{ marginBottom: "1.5rem" }}>
        ...
        <p style={{ color: "#6b7280", fontSize: "0.875rem" }}>
          Search with natural language for peaceful beaches, budget trips, or mountain adventures. Click any card to unlock local secrets!
        </p>
      </div>
      ...
      <div className="card" style={{ marginBottom: "1.25rem", padding: "1.25rem" }}>
        <div style={{ display: "flex", gap: "0.75rem", flexWrap: "wrap" }}>
          <input
            ...
            placeholder="🔍  Search destinations, tags, or describe your dream trip..."
            value={query}
            onChange={e => setQuery(e.target.value)}
            style={{ flex: 1, minWidth: "240px", fontSize: "0.9375rem" }}
          />
          <div style={{ display: "flex", gap: "0.5rem" }}>
            {[["all", "All"], ["budget", "Budget"], ["mid", "Mid-range"], ["luxury", "Luxury"]].map(([v, l]) => (
              <button key={v} onClick={() => setFilter(v)}
                style={{
                  padding: "0.5rem 1rem", borderRadius: "9999px", fontSize: "0.8125rem",
                  fontWeight: 600, cursor: "pointer", border: "1.5px solid",
                  borderColor: filter === v ? "#f47c7c" : "#e4e6ea",
                  background: filter === v ? "#f47c7c" : "white",
            ...
            <button onClick={handleAiRecommend} disabled={aiLoading}
              style={{
                padding: "0.5rem 1rem", borderRadius: "9999px", fontSize: "0.8125rem",
                fontWeight: 600, cursor: aiLoading ? "not-allowed" : "pointer", border: "none",
                background: "linear-gradient(135deg, #f47c7c, #ff9a9e)",
                color: "white", whiteSpace: "nowrap", display: "flex", alignItems: "center", gap: "0.5rem"
              }}>
      ...
      {aiResult && (
        <div style={{ background: "#fffbeb", border: "1px solid #fde68a", borderRadius: "0.75rem", padding: "1rem", marginBottom: "1.25rem", color: "#92400e", fontSize: "0.9375rem", lineHeight: 1.5 }}>
          {aiResult}
        </div>
      )}
          ...
          >
            ...
            <div className="img-card-body">
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "start", marginBottom: "0.375rem" }}>
                <div>
                  <h3 style={{ fontWeight: 700, fontSize: "1rem", color: "#1f2937" }}>{c.name}</h3>
                  <p style={{ fontSize: "0.8rem", color: "#6b7280" }}>{c.country}</p>
                </div>
                <span className={`badge ${COST_BADGE[c.cost_index]}`}>{c.cost_index}</span>
              </div>
              <p style={{ fontSize: "0.8rem", color: "#6b7280", lineHeight: 1.5, marginBottom: "0.75rem", height: "3.2em", overflow: "hidden", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical" }}>
                {c.description}
              </p>
              <div style={{ display: "flex", gap: "0.25rem", flexWrap: "wrap", marginBottom: "0.75rem" }}>
                {c.tags.slice(0, 3).map(t => (
                  <span key={t} style={{
                    background: "#f0f2f5", color: "#6b7280", padding: "0.2rem 0.5rem",
                    borderRadius: "9999px", fontSize: "0.7rem", fontWeight: 500,
                  }}>#{t}</span>
                ))}
              </div>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <span style={{ fontWeight: 700, color: "#1f2937", fontSize: "0.875rem" }}>
      ...
      {filtered.length === 0 && (
        <div className="card">
          <div className="empty-state">
            ...
            <p className="empty-state-desc">Try a different search term or filter</p>
          </div>
        </div>
      )}
